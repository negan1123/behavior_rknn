...
#ifndef PUSHSTREAMCONF_HPP_
#define PUSHSTREAMCONF_HPP_
#include <VStreamRecorderConf.hpp>

/**@class PushStreamConf
 * @brief 视频采集设置文件管理类，主要加载json配置的视频采集的参数，以提供给rtsp使用
 * */
class PushStreamConf : public GlobalConf
{
public:
	/**@brief 类的单例模式
	* @param	path	json配置文件路径
	* @return	PushStreamConf*		PushStreamConf单例
	*/
	static PushStreamConf * Instance(std::string path = std::string())
	{
		static PushStreamConf * _Instance = nullptr;
		if(nullptr == _Instance)
		{
			_Instance = new PushStreamConf(path);
		}
		return _Instance;
	}
...
private:
	...
	* @return	None
	*/
	PushStreamConf(const std::string & path){cfgPath = path;}
