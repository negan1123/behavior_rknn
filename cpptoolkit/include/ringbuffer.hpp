...
//				}
			...
			bool insert(T data)
			{
				...
				else
				{
					...
					//加个安全符号
					//data_buff[tmp_head & buffer_mask] = '\0';
					std::atomic_signal_fence(std::memory_order_release);
			...
			//替换ringbuffer的值
			size_t replaceVal(size_t index, T val, size_t count);
			 ...
			 */
			size_t readBuff(T* buff, bool f = true);
			 ...
			 * \return Number of elements that were read from internal buffer
			 */
			size_t readBuff(T* buff, size_t count, bool f = true);
		...
		size_t Ringbuffer<T, buffer_size, fake_tso, cacheline_size, index_t>::replaceVal(size_t index, T val, size_t count)
		{
			index_t available = 0;
			index_t tmp_tail = tail.load(std::memory_order_relaxed) + index;
			...
			for(size_t i = 0; i < to_read; i++)
				data_buff[tmp_tail++ & buffer_mask] = val;
	...
	template<typename T, size_t buffer_size, bool fake_tso, size_t cacheline_size, typename index_t>
		size_t Ringbuffer<T, buffer_size, fake_tso, cacheline_size, index_t>::readBuff(T* buff, bool f)
		{
			index_t available = 0;
			index_t tmp_tail = tail.load(std::memory_order_relaxed);
	...
	template<typename T, size_t buffer_size, bool fake_tso, size_t cacheline_size, typename index_t>
		size_t Ringbuffer<T, buffer_size, fake_tso, cacheline_size, index_t>::readBuff(T* buff, size_t count, bool f)
		{
			index_t available = 0;
			index_t tmp_tail = tail.load(std::memory_order_relaxed);
			size_t to_read = count;
