...
public:
	...
	*/
	int loadConfig(json &jInfo)
	{
		try
		{
			...
			getJsonInfo(apiOptions, "voice_version", voice_version);
			...
			getJsonInfo(voiceOptions, "playType", playType);
	...
	*/
	int configToJson(json &jInfo)
	{
		try
		{
			...
			apiOptions["voice_version"] = voice_version;
			...
			voiceOptions["playType"] = playType;
	...
	*/
	...
	int getPlayType() const {return playType;}
	std::string getVoiceVer() const {return voice_version;}
	...
	*/
	...
	void setPlayType(const int& type) {playType = type;}
	
	void setVoiceVer(const std::string& ver) {voice_version = ver;}
