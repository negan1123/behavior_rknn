...
{
	duration = BehaviorConf::Instance()->getBCDuration();
	frequency = BehaviorConf::Instance()->getBCFrequency();
    interval = BehaviorConf::Instance()->getVideoDuration();
}
...
void BlockCamera::inference(const time_t& time, const cv::Mat& img)
{
    ...
    if(rec.time != back.time)
    {
        ...
        recordList.push_back(rec);
        printf("遮挡摄像头:now recordList = %d\n",recordList.size());
        if(recordList.size() >= frequency)     // 当列表中图片数量大于等于 frequency 则判定为遮挡摄像头
        {
            auto it = recordList.begin() + recordList.size()/2;   // 取列表中间的图片作为报警图片
            string pic_path = BehaviorConf::Instance()->getPicVideoPath();
            pic_path += "/" + std::to_string(it->time) + "_32.jpg";   // 获取配置文件中保存地址
            cv::imwrite(pic_path, it->img); // 保存图片
            spdlog::info("触发遮挡摄像头条件");
            // 调用回调函数，将图片送至上传器
            if(cb)
                cb(pic_path);
