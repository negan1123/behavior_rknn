...
int GPIO::export_gpio()
{
    ...
    if (!exportgpio.is_open()) {
        ...
        spdlog::debug("Unable to export GPIO",this->gpionum);
	cout << endl;
        return -1;
    }
...
int GPIO::unexport_gpio()
{
    ...
    if (!unexportgpio.is_open()) {
        spdlog::debug("OPERATION FAILED:");
	spdlog::debug("Unable to unexport GPIO",this->gpionum);
	cout << endl;
        return -1;
    }
...
int GPIO::setdir_gpio(string dir)
{
    ...
    if (!setdirgpio.is_open()) {
        spdlog::debug("OPERATION FAILED:");
	spdlog::debug("Unable to set direction of GPIO",this->gpionum);
	cout << endl;
        return -1;
    }
    setdirgpio << dir;
...
int GPIO::setval_gpio(string val)
{
    ...
    if (!setvalgpio.is_open()){
 	spdlog::debug("OPERATION FAILED:");
	spdlog::debug("Unable to set value of GPIO",this->gpionum);
	cout << endl;
        return -1;
    }
    setvalgpio << val ;
...
int GPIO::get()
{
	...
	spdlog::debug("OPERATION FAILED:");
	spdlog::debug("Unable to get value of GPIO",this->gpionum);
	cout << endl;
        return -1;
    }
    getvalgpio >> val ;
