...
bool BB_cmp(ClassInfo a,ClassInfo b)
{
      return a.score < b.score;
}

// sigmoid激活函数
float BB_sigmoid(float x)
{	
	if (x >= 0)
	{
...
int BodyBehaviorDetector::calculateGridInfo(float *GridData, int a, int h, int w, float *anchors, float stride)
{	
    // 创建三类ClassInfo,用于整合到ScoreList中
    ClassInfo Phone;
    ClassInfo Head;
    BB_GridInfo Grid[a][h][w]; // 创建Grid三维数组
	...
	ssize_t size = a * w * h * 7 * sizeof(*GridData); // 初始化每个尺度的size,用于memcpy内存拷贝到Grid中
        ...
        for (int j = 0; j < h; j++)
        {
            ...
            {	
                ...
                {   
                    ...
                    x = ((BB_sigmoid(x) * 2 - 0.5) + k) * stride; // 计算x
                    y = ((BB_sigmoid(y) * 2 - 0.5) + j) * stride;
                    w = pow(BB_sigmoid(w) * 2, 2) * (*(anchors + (2 * i)));
                    h = pow(BB_sigmoid(h) * 2, 2) * (*(anchors + (2 * i + 1)));

                    // 计算得分
                    float phoneScore = Grid[i][j][k].phoneScore;
                    float headScore = Grid[i][j][k].headScore;
                    headScore = BB_sigmoid(headScore) * (conf);
                    phoneScore = BB_sigmoid(phoneScore) * (conf);

                    // 设置当前目标数据
                    setClassInfo(Phone, x, y, w, h, phoneScore);
                    setClassInfo(Head, x, y, w, h, headScore);
                    // cout<<"phone:{"<<Phone.x<<","<<Phone.y<<","<<Phone.w<<","<<Phone.h<<","<<Phone.score<<"}"<<endl;    // test
...
{	
	...
	// 以下路径后续可配置为json
    string rknn_model_path = RetinaDriverConf::Instance()->phone_rknnPath;
	string filePath = RetinaDriverConf::Instance()->ANCHORS_PATH;
	// 获取阈值
	PhoneThres = RetinaDriverConf::Instance()->phone_thresh;
...
int BodyBehaviorDetector::loadAnchors(string filePath)
{	
...
int BodyBehaviorDetector::imgPreprocess()
{	
...
int BodyBehaviorDetector::detect()
{
	int ret;
...
ClassInfo BB_xywh2xyxy(ClassInfo Info)
{	
...
ClassInfo BodyBehaviorDetector::getTrueBox(ClassInfo Info)
{	
	// 此时 info中 xywh 意义变为 x1,y1,x2,y2
	ClassInfo InfoBak = Info; // 备份ClassInfo
...
int BodyBehaviorDetector::getBestResult()
{	
	...
	if (!PhoneList.empty())
	{
		PhoneMax = *max_element(PhoneList.begin(),PhoneList.end(),BB_cmp);
		PhoneMax = BB_xywh2xyxy(PhoneMax);
...
int BodyBehaviorDetector::getBodyBehavior()
{	
...
cv::Rect BodyBehaviorDetector::getPhoneBox()
{	
	cv::Rect ret(0,0,0,0);
