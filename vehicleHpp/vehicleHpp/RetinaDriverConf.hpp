...
public:
	...
	// std::string POSE_RKNN_PATH;			///< 驾驶员姿势的模型文件路径
	// std::string POSE_MEAN;				///< 驾驶员姿势的mean参数
	// std::string POSE_STD;				///< 驾驶员姿势的std参数
	...
	*/
	int load(void)
	{
		...
		try
		{
			ifile >> jInfo;
			json retinatk;
			// json driver_behavior;
			// json pose;
			json head_behavior;
			json body_behavior;

			retinatk = jInfo.at("retinatk");
			// driver_behavior = jInfo.at("driver_behavior");
			// pose = jInfo.at("pose");
			...
			// BOD_RKNN_PATH = driver_behavior.at("rknn_path");
			// BOD_MEAN = driver_behavior.at("mean");
			// BOD_STD = driver_behavior.at("stdvalue");
			// POSE_RKNN_PATH = pose.at("rknn_path");
			// POSE_MEAN = pose.at("mean");
			// POSE_STD = pose.at("stdvalue");
