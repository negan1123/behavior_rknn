...
Yawn::Yawn()
{
	duration = BehaviorConf::Instance()->getYawnDuration();
	frequency = BehaviorConf::Instance()->getYawnFrequency();
}
...
void Yawn::inference(const time_t& time, const cv::Mat& img)
{
    count += 1;
    if (count < interval)
    {
    ...
    if(rec.time != back.time)
    {
        ...
        if(recordList.size() >= frequency)     // 当列表中图片数量大于等于 frequency 则判定为打电话
        {
            ...
            pic_path += "/" + std::to_string(it->time) + "_2.jpg";   // 获取配置文件中保存地址
