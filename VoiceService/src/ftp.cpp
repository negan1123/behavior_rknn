...
{
	curl_easy_setopt(curl, CURLOPT_URL, url);		// 设置ftp路径
	curl_easy_setopt(curl, CURLOPT_USERPWD, user_key);	// 设置用户名密码，格式(usr:pw)
	curl_easy_setopt(curl, CURLOPT_READDATA, file);		// 设置本地文件路径(上传)
	curl_easy_setopt(curl, CURLOPT_UPLOAD, 1);
	curl_easy_setopt(curl, CURLOPT_INFILESIZE, get_file_size(file));	// 设置上传文件大小
...
void curl_set_download_opt(CURL *curl, const char *url, const char *user_key, FILE *file) //设置下载配置参数
{
	curl_easy_setopt(curl, CURLOPT_URL, url);	// 设置ftp路径
	curl_easy_setopt(curl, CURLOPT_USERPWD, user_key);	// 设置用户名密码，格式(usr:pw)
	curl_easy_setopt(curl, CURLOPT_WRITEDATA, file);	// 设置本地文件路径(下载)
//	curl_easy_setopt(curl, CURLOPT_VERBOSE, 1);
}
