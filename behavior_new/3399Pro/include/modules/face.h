...
typedef struct {
	...
	/**
	 * @brief 人脸检测推理函数
	 * @param rockx_handle_t  句柄,通过rockx_create()得到
	 * @param rockx_image_t 输入图像,通过rockx_image_read()得到
	 * @param rockx_object_array_t [输出]人脸检测结果数组
	 * @param rockx_async_callback 回调函数
	 * @return rockx_ret_t
	 */
